...
p1: Virtual Pacer pace range can be set up to 14:00/km (default is 9:00/km). Run/Walk has 5 seconds increments (default is 30 seconds).
p12: p1 + In running mode the light will NOT be turned on upon each pause/lap (but can be turned on by a button, of course)
p123: p12 + '4' digit (only large one) looks better (like it is in regular Arial font).
p1234: p123 + Altitude of the starting point can be seen in paused mode under Speed, when Distance, Time, Speed and Calories are rolling.
p12345: p1234 + Heading (degrees 0-360) is shown instead of Calories in running mode. 
